...
function generateMixedMessage() {
    let randomSubject = subject[Math.floor(Math.random() * 10)];
    let randomAction = action[Math.floor(Math.random() * 10)];
    let randomAnswer = answer[Math.floor(Math.random() * 10)];
    return 'Why did the ' + randomSubject + ' ' + randomAction + '? ' + randomAnswer + '!';
}
